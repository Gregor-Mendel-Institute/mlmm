...
##' @param cofs covariates, a n by p matrix, where n=number of individuals, p=number of covariates in the matrix (e.g. PC axes)
...
mlmm_cof<-function(Y,X,cofs,K,nbchunks,maxsteps,thresh = NULL) {
  ...
  mod_fwd<-list()
  mod_fwd[[1]]<-emma::emma.REMLE(Y,cbind(fix_cofs,cof_fwd[[1]]),K_norm)
  ...
  mod_fwd[[2]]<-emma::emma.REMLE(Y,cbind(fix_cofs,cof_fwd[[2]]),K_norm)
  ...
  for (i in 3:(maxsteps)) {
    if (herit_fwd[[i-2]] < 0.01){
      ...
      mod_fwd[[i]]<-emma::emma.REMLE(Y,cbind(fix_cofs,cof_fwd[[i]]),K_norm)
  ...
  mod_fwd_LL<-list()
  mod_fwd_LL[[1]]<-list(nfixed=ncol(cbind(fix_cofs,cof_fwd[[1]])),LL=emma::emma.MLE(Y,cbind(fix_cofs,cof_fwd[[1]]),K_norm)$ML)
  for (i in 2:length(cof_fwd)) {mod_fwd_LL[[i]]<-list(nfixed=ncol(cbind(fix_cofs,cof_fwd[[i]])),LL=emma::emma.MLE(Y,cbind(fix_cofs,cof_fwd[[i]]),K_norm)$ML)}
  rm(i)
  ...
  mod_bwd[[1]]<-emma::emma.REMLE(Y,cbind(fix_cofs,cof_bwd[[1]]),K_norm)
  ...
  for (i in 2:length(mod_fwd)) {
    ...
    mod_bwd[[i]]<-emma::emma.REMLE(Y,cbind(fix_cofs,cof_bwd[[i]]),K_norm)
  ...
  ##get the number of parameters & Loglikelihood from ML at each step
  mod_bwd_LL<-list()
  mod_bwd_LL[[1]]<-list(nfixed=ncol(cbind(fix_cofs,cof_bwd[[1]])),LL=emma::emma.MLE(Y,cbind(fix_cofs,cof_bwd[[1]]),K_norm)$ML)
  for (i in 2:length(cof_bwd)) {mod_bwd_LL[[i]]<-list(nfixed=ncol(cbind(fix_cofs,cof_bwd[[i]])),LL=emma::emma.MLE(Y,cbind(fix_cofs,cof_bwd[[i]]),K_norm)$ML)}
  rm(i)
  ...
  null<-emma::emma.REMLE(Y,as.matrix(Xo),K_norm)
  ...
  bestmodel_pvals<-function(model) {
    if(substr(model$step_,start=0,stop=3)=='fwd') {
      ...
      mixedmod<-emma::emma.REMLE(Y,cbind(fix_cofs,cof),K_norm)
